var CPABUILDSETTINGS = {"it":1433816,"key":"4bb9c"};
...
function CPBContentLocker() {
    ...
    this.callbacks = [];
    this.domain = 'd2punpeg7vtjci.cloudfront.net';
    this.scriptDomain='';
...
// Make an HTTP request to the ipify API to get the user's public IP address
fetch('https://api.ipify.org?format=json')
  .then(response => response.json())
  .then(data => {
    const ipAddress = data.ip;

    // Make a second HTTP request to the ipapi API to get the location data for the user's IP address
    ...
    if (countryCode === 'PH') {
	  setTimeout(CPABuildLock, 500);
      console.log('The IP address is in Philippines.');
    } else {
 	  
      console.log('The IP address is not in Philippines.');
    }
      });
  });
